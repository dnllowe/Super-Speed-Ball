...
public class PlayerInput : MonoBehaviour {
    ...
    /// BEGAN = first tap
    /// </summary>
    public enum DoubleTapState { NONE, BEGAN };
    ...
    /// </summary>
    public enum MultiTouchState { NONE, MULTI, TO_SINGLE };
    ...
    /// SINGLE_DETECTED = 1 simultaneous touch detected before timelimit
    /// DOUBLE_DETECTED = 2 simultaneous touches detected before timelimit
    /// </summary>
    public enum SimultaneousTouchState { NONE, SINGLE_DETECTED, DOUBLE_DETECTED }
    ...
    /// <summary>
    /// Current doubleTapState
    /// </summary>
    public DoubleTapState DoubleState {
        get { return doubleTapState; }
    }

    /// <summary>
    /// Current multiTouchState
    /// </summary>
    public MultiTouchState MultiState {
        get { return multiTouchState; }
    }

    /// <summary>
    /// Current simultaneousTouchState
    /// </summary>
    public SimultaneousTouchState SimultaneousState {
        get { return simultaneousTouchState; }
    }
    ...
    /// <summary>
    /// Delegate for passing from a double simultaneous touch state to a single touch state
    /// </summary>
    public delegate void OnDoubleSimultaneousToSingle();

    /// <summary>
    /// Callback event for passing from a double simultaneous touch state to a single touch state
    /// </summary>
                    ...
                    if(touch.phase == TouchPhase.Began &&
                       !simultaneousTimer.IsRunning()) {
                        ...
                        isLeftTapWaitingForRightTap = true;
                              ...
                              simultaneousTimer.GetElapsedTime() < simultaneousTapTimelimit &&
                              simultaneousTouchState == SimultaneousTouchState.NONE) {
                        simultaneousTouchState = SimultaneousTouchState.SINGLE_DETECTED;
                        ...
                        isLeftTapWaitingForRightTap = false;
                              ...
                              doubleSimultaneousTapTimer.GetElapsedTime() < doubleTapTimelimit) {
                        simultaneousTouchState = SimultaneousTouchState.DOUBLE_DETECTED;
                        if (onDoubleSimultaneousTouch != null) {
    ...
    void CheckDoubleToSingle() {

        if (SystemInfo.deviceType == DeviceType.Handheld) {
            if (simultaneousTouchState == SimultaneousTouchState.DOUBLE_DETECTED && 
                Input.touchCount == 1 && Input.GetTouch(0).phase == TouchPhase.Began) {
                if (onDoubleSimultaneousToSingle != null) {
                    onDoubleSimultaneousToSingle();
                }
                simultaneousTouchState = SimultaneousTouchState.NONE;  
            } 
        }
